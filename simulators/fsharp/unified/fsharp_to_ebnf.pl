% fsharp_to_ebnf.pl — Convert fsharp_parser.pl DCG rules to Mermaid railroad diagrams
%
% Usage:
%   swipl -l fsharp_to_ebnf.pl -g main -t halt

:- module(fsharp_to_ebnf, [main/0]).
...
main :-
    ...
    extract_dcg_rules('fsharp_parser.pl', Rules),
    group_rules(Rules, Grouped),
    filter_productions(Options, Grouped, Selected),
    generate_markdown(Selected, MD),
    setup_call_cleanup(
        open('fsharp_railroad.md', write, Out),
        write(Out, MD),
        close(Out)
    ),
    length(Selected, N),
    format("Generated fsharp_railroad.md with ~w production diagrams.~n", [N]).
...
extract_dcg_rules(File, Rules) :-
    % Save and restore the double_quotes flag
...
is_dcg_rule((Head --> _Body)) :-
    callable(Head),
    functor(Head, Name, _),
    \+ member(Name, [ws, ws_no_nl, kw, ident, ident_rest, number, digits,
                     forbidden_start, is_keyword,
...
filter_productions(top, Grouped, Selected) :-
    TopNames = [program, binding, expr, arith_expr, term, factor, app_expr, primary],
...
generate_markdown(Productions, MD) :-
    ...
    atomic_list_concat(['# F# Language - Railroad Diagrams\n\n',
