...
for AutomataLib (1D cellular automaton).

Runs both:
1. CDB attached to Python loading AutomataLib.dll (compiled Clarion)
2. SWI-Prolog interpreter executing AutomataLib.clw source
...
def run_prolog_trace():
    ...
    result = subprocess.run(
        ["swipl", "-g", "main,halt", "-t", "halt(1)", "traces/trace_automata.pl"],
...
def main():
    print("=" * 60)
    print("AutomataLib: CDB vs Prolog Trace Comparison")
    print("=" * 60)
